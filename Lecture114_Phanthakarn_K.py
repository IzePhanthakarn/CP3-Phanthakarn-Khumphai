...
def calculate_click():
    ...
    try:
        convert_start_usd = "{0:.2f}".format(c.convert(
            1, 'USD', 'THB', date=date(int(start_year), int(start_month), int(start_day))))
        start_usd.configure(text="1 USD : "+convert_start_usd+" THB")
        error_start_date.configure(text="")
    except:
        error_start_date.configure(text="no data start")
        start_usd.configure(text="")
        summarize_THB.configure(text="")
        summarize_percentage.configure(text="")

    try:
        convert_end_usd = "{0:.2f}".format(c.convert(
            1, 'USD', 'THB', date=date(int(end_year), int(end_month), int(end_day))))
        end_usd.configure(text="1 USD : "+convert_end_usd+" THB")
        erorr_end_date.configure(text="")
    except:
        erorr_end_date.configure(text="no data end")
        end_usd.configure(text="")
        summarize_THB.configure(text="")
        summarize_percentage.configure(text="")

    try:
        summarize_total_THB = "{0:.2f}".format(
            float(convert_end_usd) - float(convert_start_usd))
        ...
        summarize_total_percentage = "{0:.2f}".format(
            ((float(convert_end_usd) - float(convert_start_usd))/float(convert_start_usd))*100)
...
head_lebel = tk.Label(main_window, text="USD History", background='black',
                      foreground='white', font=('Helvetica', 22, 'bold'))
head_lebel.place(x=150, y=30, width=200, height=40)

start_date = tk.Label(main_window, text="select start date", background='black',
                      foreground='white', font=('Helvetica', 16, 'bold'))
start_date.place(x=50, y=100, width=170, height=30)

end_date = tk.Label(main_window, text="select end date", background='black',
                    foreground='white', font=('Helvetica', 16, 'bold'))
...
start_usd = tk.Label(main_window, text="")
start_usd.place(x=85, y=300, width=100)

end_usd = tk.Label(main_window, text="")
end_usd.place(x=315, y=300, width=100)
